...
9. ✅ **Schema Evolution** — `src/format/Reader.luau` now dispatches on the format version field to frozen per-version readers (`src/format/readers/V1.luau`, `V2.luau`) and pipes the parsed result through `compat.migrate` — previously the compat registry existed but was never called, and `Reader.luau` hard-asserted `version == 1`. Uses the templateRefs raw-varint → RLE change as the first real (not synthetic) v1→v2 bump, proven against a frozen v1 binary fixture (`tests/fixtures/format_v1_sample.bin`, generated once before the version bump landed, never regenerated). `Writer.luau` only ever writes the current version (2).
...
Lattice is feature-complete for all six previously-planned specs (extended codec coverage, column dictionary encoding, schema evolution, CLI tooling, incremental/delta encoding) plus the earlier Roblox package/perf/docs polish. Nothing is actively in progress. Suggested next steps, not yet scoped as specs:

1. **Reconcile duplicate identity-matching logic**: `cli/Diff.luau`'s `matchInstances` and `src/patch/Identity.luau` were built independently in isolated worktrees and solve the same UniqueId-with-positional-fallback problem. Unify into one shared module.
2. **`cli diff`'s cross-class-match gap**: when positional fallback pairs two instances of different classes, `cli/Diff.luau` currently drops the pair silently instead of reporting a class-mismatch. Low severity, flagged in code review.
3. **Whole-instance dedup removal**: `bench/DedupVsDictionary.luau` showed no measurable benefit from `templateRefs` dedup once Dictionary encoding was active, on one fixture. Re-measure on larger/more realistic fixtures before deciding whether to remove it (see Completed Milestone 8).
